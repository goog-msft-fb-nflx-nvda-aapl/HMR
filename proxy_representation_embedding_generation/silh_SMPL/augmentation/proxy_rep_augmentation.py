...
def augment_proxy_representation(orig_segs, orig_joints2D, proxy_rep_augment_params, augment=True):
    if augment:
        new_segs = orig_segs.clone()
        new_joints2D = orig_joints2D.clone()

        if proxy_rep_augment_params['remove_appendages']:
            new_segs = random_remove_bodyparts(new_segs, classes_to_remove=proxy_rep_augment_params['remove_appendages_classes'], probabilities_to_remove=proxy_rep_augment_params['remove_appendages_probabilities'])
        if proxy_rep_augment_params['occlude_seg']:
            new_segs = random_occlude( new_segs, occlude_probability=proxy_rep_augment_params['occlude_probability'], occlude_box_dim=proxy_rep_augment_params['occlude_box_dim'] )
        if proxy_rep_augment_params['deviate_joints2D']:
            new_joints2D = random_joints2D_deviation(new_joints2D, delta_j2d_dev_range=proxy_rep_augment_params['delta_j2d_dev_range'], delta_j2d_hip_dev_range=proxy_rep_augment_params['delta_j2d_hip_dev_range'])

        return new_segs, new_joints2D
    else:
        return orig_segs, orig_joints2D